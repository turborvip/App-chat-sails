...
const getNewToken = (req,res) => {
    ...
    if(refreshToken && (refreshTokens.includes(refreshToken))) {
        jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, (err, data) => {
            if (err) {
                return res.status(403).json({ errRefreshToken: true, msg:'Authorization failed - Refresh token expired' });
            }
        ...
        res.status(404).json({errRefreshToken:true, msg:'Refresh Token not found'});        
    }
}
